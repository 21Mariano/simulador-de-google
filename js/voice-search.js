...
const voiceSearchModalClose = () =>{
	...
	setTimeout( () => voiceSearch.style.display = "none",250)
}
...
const voiceRecognition = () =>{
	...
	if (microAceptado == false) {
		...
		if (!'SpeechRecognition' in window) alert("que pena, no podes usar la API");
	};
    ...
    recognition.onresult = (event) => {
		...
		recognition.stop();
		setTimeout(()=>window.open("https://google.com/search?q="+voiceText),1800);
	};
